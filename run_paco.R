...
# run paco
paco_run <- PACo(D, nperm=999, seed=909, method='quasiswap', symmetric=TRUE) #Symetric - is one meant to track the evolution of another?
# Get interaction-specific cophylogenetic contributions using jacknifing
paco_run <- paco_links(paco_run,  .parallel = TRUE)

write_rds(paco_run, "paco_psyllid_microbe.rds")
...
write_rds(paco_run, "paco_trioza_microbe.rds")


# paco psyllid microbe filtered -------------------------------------------
...
write_rds(paco_run, "paco_psyllid_microbe_filtered.rds")
...
write_rds(paco_run, "paco_trioza_microbe_filtered.rds")
