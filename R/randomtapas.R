...
# of the R session)
TreeH <- read.nexus("PfxCP_Data/Pfluviatilis.tre")	      # Amphipod (host) consensus tree
TreeS <- read.nexus("PfxCP_Data/Cparvum.tre")	      # Trematode (symbiont) consensus tree
mTreeH <- read.nexus("PfxCP_Data/m_Pfluviatilis.tre") # 1,000 post. prob. amphipod trees
mTreeS <- read.nexus("PfxCP_Data/m_Cparvum.tre")	   # 1,000 post. prob. trematode trees
HS <- as.matrix(read.table("PfxCP_Data/PfxCp_assoc.txt", header = TRUE)) # Assoc. matrix
#
