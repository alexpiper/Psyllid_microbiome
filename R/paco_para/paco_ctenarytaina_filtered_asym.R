...
# paco ctenarytaina microbe -----------------------------------------------------

#Prepare co-occurance matrix
coocur <- ps3 %>%
  subset_samples(psyllid_genus == "Ctenarytaina") %>%
...
# run paco
print("running paco")
paco_run <- PACo(D, nperm=1000, seed=909, method='quasiswap', symmetric=FALSE)

paco_run <- paco_links(paco_run, .parallel = TRUE)

write_rds(paco_run, "paco_ctenarytaina_microbe_filtered_asym.rds")
