...
print("running paco")
# run paco
paco_run <- PACo(D, nperm=1000, seed=909, method='quasiswap', symmetric=FALSE) #Symetric - is one meant to track the evolution of another?
# Get interaction-specific cophylogenetic contributions using jacknifing
paco_run <- paco_links(paco_run, .parallel = TRUE)

write_rds(paco_run, "paco_psyllid_microbe_filtered_asym.rds")
