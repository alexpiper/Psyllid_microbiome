...
find $(/usr/bin/ls -d $PWD/*) -name 'paco_*' -type f > sequence_index.txt
...
coocur <- ps3_filt %>%
    subset_samples(psyllid_genus %in% c("Trioza", "Ctenarytaina", "Psylla")) %>%
...
# Read in Paco runs 
paco_run_trioza <- readRDS("output/cophylogeny/paco_trioza_microbe_filtered_asym.rds")
...
links <- do.call("list", mget(grep("paco_run_",names(.GlobalEnv),value=TRUE))) %>%
  purrr::map(function(x){
    ...
    data.frame(
      joint=names(x$jackknife$mean),
      values=unname(x$jackknife$mean),
...
test <- colnames(coocur) %>%
  enframe(name=NULL, value="OTU")%>%
  left_join(links)
...
# Parafit run
PF_run_trioza <- readRDS("output/cophylogeny/parafit_trioza_microbe_filtered.rds")
...
PF_run_trioza$ParaFitGlobal
PF_run_trioza$p.global
...
PF_links <- do.call("list", mget(grep("PF_run_",names(.GlobalEnv),value=TRUE))) %>%
  purrr::map(function(x){
    genus_name <- x$para.per.host[1] %>% str_remove("_.*$")
...
gg.heatmap <- heatmap_dat %>%
  ...
  dplyr::select(Sample_Name, OTU, genus, highlight, pos_x, pos_y) %>%
  dplyr::mutate(Sample_Name = factor(Sample_Name),
                OTU = factor(OTU),
                genus=factor(genus, levels=c("Psylla" ,"Trioza", "Ctenarytaina")))  %>%
...
#  transform_sample_counts(function (x) x/sum(x)) %>%
#  speedyseq::psmelt() %>%
#  filter(genus %in% c("Arsenophonus")) %>%
#  dplyr::select(OTU, psyllid_spp, SampleID, Abundance) %>%
#  mutate(OTU = OTU %>%
#           str_replace_all(" |-", "_")) %>%
#  dplyr::group_by(OTU, psyllid_spp) %>%
