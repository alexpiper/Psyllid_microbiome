...
samdf <- read_csv("sample_data/Sample_info.csv")  %>%
  ...
  mutate(psyllid_spp = str_remove(psyllid_spp, "\\."))%>%
  mutate(hostplant_spp = str_remove(hostplant_spp, "\\."))
...
psyllid_tree$tip.label <- psyllid_tree$tip.label %>%
  ...
  str_replace_all(pattern="Cryptoneossa triangula", replacement="Cryptoneossa sp") %>% #check which is best
  str_replace_all(pattern="Anoeconeossa communis", replacement="Anoeconeossa sp") %>%  #same
  trimws(which="right")
...
# Matrix correlations
#only use samples present in all
samples <- Reduce(intersect, list(rownames(otu_table(ps2_noent)), colnames(phylo.dist), colnames(coi.dist), colnames(plant.dist), colnames(spat.dist)))
...
#Rename taxa
taxa_names(ps3) <- paste0("SV", seq(ntaxa(ps3)),"-",tax_table(ps3)[,8])
...
```{r carsonella}
ps3.carson <- ps3 %>% 
  subset_taxa(genus == "Candidatus Carsonella") %>%
  filter_taxa(function(x) mean(x) > 0, TRUE) #Drop missing taxa from table 
taxa_names(ps3.carson) <- paste0("SV", seq(ntaxa(ps3.carson)),"-",tax_table(ps3.carson)[,8])

# Need to merge species first

# MAybe flag top carsonella, then subset the phyloseq object to just these
# See without these first, to verify how LULU has done for removing carsonella?

# Get cophenetic distance
h_dist <- cophenetic(pruned.tree)
s_dist <- cophenetic(phy_tree(ps3.carson))

#Prepare co-occurance matrix
samples <- intersect(sample_names(ps3.carson), rownames(h_dist))

otu <- ps3.carson %>%
    subset_samples(sample_names(ps3) %in% samples) %>%
    otu_table %>%
    as.matrix() %>% 
  apply(2, function(x) ifelse(x > 0, 1, 0)) 
...
as.data.frame() %>%
rownames_to_column("Sample_Name") %>%
left_join(as_data_frame(sample_data(ps3.carson)) %>% dplyr::select(Sample_Name, psyllid_spp)) %>%
pivot_longer(cols=starts_with("SV"),names_to="OTU", values_to = "val") %>%
filter(val > 0) %>%
mutate(psyllid_spp = str_replace_all(psyllid_spp,"'","")) %>%
dplyr::select(psyllid_spp, OTU) %>%
as.matrix()

otu.lut <- which(otu ==1, arr.ind=TRUE)
assoc <- cbind(rownames(otu)[otu.lut[,1]], colnames(otu)[otu.lut[,2]])

# Should weights be from link strength instead?
weight <- as.data.frame(assoc) %>% 
  magrittr::set_colnames(c("Sample_Name", "OTU")) %>%
  left_join(res %>% dplyr::select(Sample_Name, OTU, values)) %>%
 # mutate(values = (values^-2)/50) %>%
  pull(values)
...
plot.cophylo(obj, link.lwd=weight*5, link.lty=2, fsize=0.5, pts=FALSE, link.type="curved")
