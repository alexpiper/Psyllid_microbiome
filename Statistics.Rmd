...
#Read filtered
paco_run <- readRDS("paco_psyllid_microbe_filtered.rds")
...
# Parafit
#Test paragit
PF <- parafit(h_dist, s_dist, coocur, nperm=99, test.links=FALSE, silent=TRUE)
...
# run paco
paco_run <- PACo(D, nperm=1000, seed=92409, method='quasiswap', symmetric=TRUE)
...
#Test paragit
PF <- parafit(h_dist, s_dist, coocur, nperm=99, test.links=TRUE, silent=TRUE)
...
# Read in filtered
paco_run <- readRDS("paco_trioza_microbe_filtered.rds")
...
pdf(file="figs/trioza_microbiome_fit_summary.pdf",  width = 8, height = 11, paper="a4")
  plot(gg.hlinks + gg.slinks)
try(dev.off(), silent=TRUE)
```
