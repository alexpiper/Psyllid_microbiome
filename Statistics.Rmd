...
tax_table(ps2) %>%
  ...
  dplyr::filter(genus == "Candidatus Carsonella") %>%
...
speedyseq::tax_glom(ps2, taxrank="genus")  %>%
  ...
  summarise(mean = mean(Abundance), 
            ...
            n_species = n_distinct(psyllid_spp))
...
broom::tidy(TukeyHSD(aov(alpha ~psyllid_genus, data=div_table2)))

report::report(aov(Shannon ~psyllid_genus, data=div_table2))
broom::tidy(TukeyHSD(aov(Shannon ~psyllid_genus, data=div_table2)))

report::report(aov(pd ~psyllid_genus, data=div_table2))
broom::tidy(TukeyHSD(aov(pd ~psyllid_genus, data=div_table2)))

# Difference between genus/geography factors ANOVA
report::report(aov(alpha ~genus_geo, data=div_table2))
broom::tidy(TukeyHSD(aov(alpha ~genus_geo, data=div_table2)))

report::report(aov(Shannon ~genus_geo, data=div_table2))
broom::tidy(TukeyHSD(aov(alpha ~genus_geo, data=div_table2)))

report::report(aov(alpha ~genus_geo, data=div_table2))
broom::tidy(TukeyHSD(aov(pd ~genus_geo, data=div_table2)))
...
broom::tidy(TukeyHSD(aov(alpha ~psyllid_genus, data=div_table_rare2)))

report::report(aov(Shannon ~psyllid_genus, data=div_table_rare2))
broom::tidy(TukeyHSD(aov(Shannon ~psyllid_genus, data=div_table_rare2)))

report::report(aov(pd ~psyllid_genus, data=div_table_rare2))
broom::tidy(TukeyHSD(aov(pd ~psyllid_genus, data=div_table_rare2)))

# Difference between genus/geography factors ANOVA
report::report(aov(alpha ~genus_geo, data=div_table_rare2))
broom::tidy(TukeyHSD(aov(alpha ~genus_geo, data=div_table_rare2)))

report::report(aov(Shannon ~genus_geo, data=div_table_rare2))
broom::tidy(TukeyHSD(aov(alpha ~genus_geo, data=div_table_rare2)))

report::report(aov(alpha ~genus_geo, data=div_table_rare2))
broom::tidy(TukeyHSD(aov(pd ~genus_geo, data=div_table_rare2)))
...
broom::tidy(TukeyHSD(aov(alpha ~psyllid_genus, data=div_table_subset2)))

report::report(aov(Shannon ~psyllid_genus, data=div_table_subset2))
broom::tidy(TukeyHSD(aov(Shannon ~psyllid_genus, data=div_table_subset2)))

report::report(aov(pd ~psyllid_genus, data=div_table_subset2))
broom::tidy(TukeyHSD(aov(pd ~psyllid_genus, data=div_table_subset2)))

# Difference between genus/geography factors ANOVA
report::report(aov(alpha ~genus_geo, data=div_table_subset2))
broom::tidy(TukeyHSD(aov(alpha ~genus_geo, data=div_table_subset2)))

report::report(aov(Shannon ~genus_geo, data=div_table_subset2))
broom::tidy(TukeyHSD(aov(alpha ~genus_geo, data=div_table_subset2)))

report::report(aov(alpha ~genus_geo, data=div_table_subset2))
broom::tidy(TukeyHSD(aov(pd ~genus_geo, data=div_table_subset2)))
```	
...
## Dissimilarities between major groups

```{r Avg Dissimiarity}
install_github("pmartinezarbizu/pairwiseAdonis/pairwiseAdonis")
library(pairwiseAdonis)

# NOTE could be a nested design - in which case potentiall adonis <- adonis(dist ~ hosplant_spp / psyllid_spp, strata = hosplant_spp, data = df_compare)

major_metadata <- metadata %>% 
  dplyr::filter(psyllid_genus %in% c("Trioza", "Ctenarytaina", "Psylla"))

dist_major <- distlist$Aitchison[major_metadata$Sample_Name, major_metadata$Sample_Name]

broom::tidy(adonis(dist_major ~ hostplant_spp / psyllid_genus / psyllid_spp, method="euclidean",
                   data=major_metadata)$aov.tab)

broom::tidy(adonis(dist_major ~ psyllid_spp * hostplant_spp * psyllid_genus, method="euclidean",
                   data=major_metadata)$aov.tab)

#Just not much captured by the psyllid_genus compared to the others. See if i can formalise this analysis in a model

pairwise.adonis2(dist_major ~ psyllid_genus, method="euclidean",
                   data=major_metadata)

# Double check directionality?
test_metadata <- metadata %>% 
  dplyr::filter(psyllid_genus %in% c("Psylla"))
test <- distlist$Aitchison[test_metadata$Sample_Name, test_metadata$Sample_Name]

test<- meandist(as.dist(dist_major), grouping = psyllid_genus)
dune.md


broom::tidy(adonis(dist_major ~ genus_geo, method="euclidean",
                   data=major_metadata)$aov.tab)

pairwise.adonis2(dist_major ~ genus_geo, method="euclidean",
                   data=major_metadata)
