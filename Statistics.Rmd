...
ggtree(treelist) + facet_wrap(~.id, scale="free") + theme_tree2()
...
for (d in 1:length(dists)){
  ...
  for (i in 1:length(slices)){
    ...
    #Jac
    M <- mantel(Betas[nslice,"Jac",samples_to_use,samples_to_use], test.dist[samples_to_use,samples_to_use])
    man_jac <- data.frame(metric="Jac", test=dists[d], stat=M$statistic,  signif=M$signif)
    #Bray
    M <- mantel(Betas[nslice,"Bray",samples_to_use,samples_to_use], test.dist[samples_to_use,samples_to_use])
    man_bray <- data.frame(metric="Bray", test=dists[d], stat=M$statistic,  signif=M$signif)
    #Aitch_psedo
    M <- mantel(Betas[nslice,"Aitch_pseudo",samples_to_use,samples_to_use], test.dist[samples_to_use,samples_to_use])
    man_aitch_pseudo <- data.frame(metric="Aitch_pseudo", test=dists[d], stat=M$statistic,  signif=M$signif)
    #Aitch_rep
    M <- mantel(Betas[nslice,"Aitch_rep",samples_to_use,samples_to_use], test.dist[samples_to_use,samples_to_use])
    man_aitch_rep <- data.frame(metric="Aitch_rep", test=dists[d], stat=M$statistic,  signif=mantel$signif)
...
for (d in 1:nrow(dists)){
  ...
  for (i in 1:length(slices)){
    ...
    nslice <- as.character(slices[i])
    #Jac
    PM <- mantel.partial(Betas[nslice,"Jac",samples_to_use,samples_to_use],
                         test.dist[samples_to_use,samples_to_use],
                          control.dist[samples_to_use,samples_to_use])
    PM_jac <- data.frame(metric="Jac", dist1=dists$dist1[d], dist2=dists$dist2[d], stat=PM$statistic,  signif=PM$signif)
    #Bray
    PM <- mantel.partial(Betas[nslice,"Bray",samples_to_use,samples_to_use],
                         test.dist[samples_to_use,samples_to_use],
                         control.dist[samples_to_use,samples_to_use])
    PM_bray <- data.frame(metric="Bray", dist1=dists$dist1[d], dist2=dists$dist2[d], stat=PM$statistic,  signif=PM$signif)
    #Aitch_psedo
    PM <- mantel.partial(Betas[nslice,"Aitch_pseudo",samples_to_use,samples_to_use], 
                         test.dist[samples_to_use,samples_to_use], 
                         control.dist[samples_to_use,samples_to_use])
    PM_aitch_pseudo <- data.frame(metric="Aitch_pseudo", dist1=dists$dist1[d], dist2=dists$dist2[d], stat=PM$statistic,  signif=PM$signif)
    #Aitch_rep
    PM <- mantel.partial(Betas[nslice,"Aitch_rep",samples_to_use,samples_to_use], 
                         test.dist[samples_to_use,samples_to_use], 
                         control.dist[samples_to_use,samples_to_use])
    PM_aitch_rep <- data.frame(metric="Aitch_rep", dist1=dists$dist1[d], dist2=dists$dist2[d], stat=PM$statistic,  signif=PM$signif)
    manlist[[i]] <- bind_rows(PM_jac, PM_bray, PM_aitch_pseudo, PM_aitch_rep) %>%
      mutate(slice=slices[i])
  }
  mantlist_distances[[d]] <- bind_rows(manlist)
}
...
obj <- cophylo(TreeH,TreeS, linkhs, rotate=FALSE) # Rotate crashes it, because too many nodes?
