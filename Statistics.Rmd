...
## Beta diversity association over Phylogenetic scale 

```{r agglomeration}

## Define phy_glom function - alternative to tip_glom
library(castor)
phy_glom <- function(physeq, similarity = 0.2) 
{
  ...
  if(length(DescendingTips)>0){
    for (i in 1:length(DescendingTips)) {
      physeq <- phyloseq::merge_taxa(physeq, eqtaxa = DescendingTips[[i]])
    }
  }
  
  Ncollapsed <- length(DescendingTips)
  NcollapsedOld <- sum(do.call(rbind, lapply(DescendingTips, function(x) length(x))))
  message(paste(NcollapsedOld, "OTUS within", Ncollapsed, "Nodes collapsed" ))
  return(physeq)
}
...
distances <-  c("ps2.philr", "ps2.clr", "ps2.bray", "ps2.jac")
...
adlist_distances <-  vector("list", length=length(distances))
mantlist_distances <-  vector("list", length=length(distances))
...
ps2.jac <- vegdist(otu_table(ps.glom),method="jac",binary = T)
  ...
  mantlist[[d]] <- bind_rows(man_phylo, man_geo, man_plant,
  ...
  adlist_distances[[i]] <- bind_rows(adlist)  %>%
    mutate(glom = glomseq[i])
  mantlist_distances[[i]] <- bind_rows(mantlist)   %>%
    mutate(glom = glomseq[i])
...
# Trees
library(ggtree)
trees <- lapply(treelist, phy_tree)
class(trees) <- "multiPhylo"
gg.glomtrees <- ggtree(trees) + facet_wrap(~.id, scale="free", nrow=1) + theme_void()

#Combine
library(patchwork)

gg.glomtrees / gg.mantel 
...
```{r similarity slicing}

tree <- phy_tree(ps2)

# slice tree at different total branch lengths (using ape)

# Get tip labels that are still in tree

# Test
 dd = as.dist(cophenetic.phylo(phy_tree(ps2)))
