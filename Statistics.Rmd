...
# psyllid_tree
tree1 <- obj[["trees"]][[1]]
p1 <- ggtree(tree1, ladderize=FALSE, aes(colour=new_vals)) + geom_tiplab()
weights_p1 <- p1$data %>%
  left_join(res %>%
...
weights_p1 <- weights_p1 %>%
  left_join(data.frame(node=weights_p1$node, new_vals = weights_p1$node %>% purrr::map_dbl(average_descendants, tree=tree1, df=weights_p1)))

p1 <- p1 %<+% weights_p1 + geom_tippoint(aes(colour=new_vals)) +  scale_color_gradient(low="steelblue", high="darkorange1")

# OTU tree
tree2 <- obj[["trees"]][[2]]
p2 <- ggtree(tree2, ladderize=TRUE, aes(colour=new_vals))
weights_p2 <- p2$data %>%
  left_join(res %>% 
    group_by(OTU) %>%
    summarise(values = mean(values)) %>%
...
weights_p2 <- weights_p2 %>%
  left_join(data.frame(node=weights_p2$node, new_vals = weights_p2$node %>% purrr::map_dbl(average_descendants, tree=tree1, df=weights_p2)))

p2 <- p2 %<+% weights_p2  + geom_tippoint(aes(colour=new_vals)) +  scale_color_gradient(low="steelblue", high="darkorange1")
...
# OTU tree
tree2 <- obj[["trees"]][[2]]
p2 <- ggtree(tree2, ladderize=FALSE, aes(colour=new_vals))
weights_p2 <- p2$data %>%
  left_join(links %>% 
    group_by(OTU) %>%
    summarise(values = mean(values)) %>%
...
# psyllid_tree
tree2 <- obj[["trees"]][[2]]
p2 <- ggtree(tree2, ladderize=FALSE, aes(colour=new_vals)) + geom_tiplab()
weights_p2 <- p2$data %>%
  left_join(links %>% 
    group_by(psyllid_spp) %>%
    summarise(values = mean(values)) %>%
...
weights_p2 <- weights_p2 %>%
  left_join(data.frame(node=weights_p2$node, new_vals = weights_p2$node %>% purrr::map_dbl(average_descendants, tree=tree2, df=weights_p2)))
