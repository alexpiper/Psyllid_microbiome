...
.cran_packages <- c("ggplot2", "gridExtra", "tidyverse", "scales", "stringdist", "patchwork", "vegan", "ggpubr", "seqinr", "viridis", "ape", "sp", "data.table", "RColorBrewer", "ggtree", "castor", "abind", "Matrix", "minpack.lm", "Hmisc")
...
tax <- readRDS("output/rds/tax_RDP_final.rds") 
...
## box and whisker plots of pairwise similarity between reps, species, species~hostplant
ps.reps

# Distance between replicates
samples <- unique(sample_data(ps.reps)$Sample.Name)
repdist <- vector("list", length=length(samples))
for (i in 1:length(samples)){
  dist <- ps.reps %>% 
    subset_samples(Sample.Name ==samples[i]) %>%
...
repdist <- bind_rows(repdist)

# Distance between species
psyllid_spp <- unique(sample_data(ps1)$psyllid_spp)
sppdist <- vector("list", length=length(psyllid_spp))
for (i in 1:length(psyllid_spp)){
  dist <- ps1 %>% 
    subset_samples(psyllid_spp == psyllid_spp[i]) %>%
    filter_taxa(function(x) mean(x) > 0, TRUE) %>%
    otu_table() 
...
dist <- ps1 %>% 
    subset_samples(Sample.Name %in%  c("93", "94","106", "107","112", "113")) %>%
    filter_taxa(function(x) mean(x) > 0, TRUE) %>%
    otu_table()  %>%
    vegan::vegdist(method="jaccard")
...
dist2 <- ps1 %>% 
    subset_samples(Sample.Name %in%  c("200", "94","106", "107","112", "113")) %>%
    filter_taxa(function(x) mean(x) > 0, TRUE) %>%
    otu_table()  %>%
    vegan::vegdist(method="jaccard")
...
Perhaps use a more traditional estimator such as picante to just get it done
...
Betas=BDTT(similarity_slices = slices, tree = phy_tree(ps2), sampleOTUs = t(otu_table(ps2)), metrics=c("Bray","Jaccard","Aitchison","Philr", "Unifrac", "WUnifrac"), zeroes="pseudo", parallel=FALSE, quiet=FALSE)
```
