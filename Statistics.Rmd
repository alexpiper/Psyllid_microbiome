...
#samples <- intersect(rownames(seqtab), rownames(tax))
#seqtab <- seqtab[samples,]
#tax <- tax[samples,]
...
BTDD_topo <- Betas %>%
  purrr::map_depth(.depth=2, function(x){
    run_tree_comparison(x, dists=c("phylo.dist","coi.dist","plant.dist",
                          "newplant.dist", "spat.dist"), #Change these to matrices
               samples=samples,
               clust.method="average",
               measure="all",
               runs=100)
...
h_dist <- cophenetic(pruned.tree2)
...
res <- data.frame(OTU=names(paco_residuals), values=unname(paco_residuals)) %>%
  ...
  mutate(psyllid_genus = str_extract(Sample_Name, "^(.*?)(?=_)"))

ggplot(res, aes(x=values))+
  geom_density(fill='grey70')+
  geom_vline(data=res %>% 
               filter(genus=="Candidatus Carsonella"), aes(xintercept=values)) +
  #facet_wrap(~order) +
  theme_bw()+
...
library(phytools)
obj <- cophylo(tr1=newtree, tr2=new_stree, assoc=assoc, rotate=FALSE) 
plot.cophylo(obj, link.lwd=weights*6, link.lty="solid", fsize=0.1, pts=FALSE, link.type="straight", link.col="steelblue")
dev.off() #reset graphics device


#Model bith the residuals and links by something, phylum?
simple_model <- glmer(values ~ order + (1|psyllid_genus), data=res)
summary(simple_model)

# Is there a way to set the intercept specifically as carsonella? THat would be a good test to show that 

test <- parameters::parameters(simple_model)
check_model(simple_model)

plot(check_distribution(simple_model))

# plot this on the bacterial and colour the the branches by weight to highlight those with the highest importance?
# Could do with GGtree, then do the same on the psyllid tree.
# Make a gglineplot just lining 2 sides together, stitch the 3 into 1 using patchwork
test <- res %>% 
  group_by(OTU) %>%
  summarise(values = mean(values))  

ggplot(res, aes(x=values, y=family)) + 
  geom_point()
...
## Look at differences between 2

plants <- read_csv("sample_data/plant_distance.csv") %>% 
  dplyr::select(SampleID, `Host plants`) %>%
  full_join(samdf %>% dplyr::select(SampleID, hostplant_spp, psyllid_spp)) %>%
  dplyr::rename(hostplant_spp_dist = `Host plants`,
                hostplant_spp_sampledata = hostplant_spp) %>%
  dplyr::mutate(hostplant_spp_dist = hostplant_spp_dist %>% str_remove(".$"))

write_csv(plants, "hostplant_species.csv")

# use phylocom to get a tree
plants <- read_csv("sample_data/plant_distance.csv")  %>%
  mutate(host_plants = `Host plants` %>% str_remove(".$")) %>%
  pull(host_plants) %>%
  unique


library(brranching)
tree <- brranching::phylomatic(plants, get="POST")

sp_list_phylocom = brranching::phylomatic_names(plants, 
                                                format = "isubmit", 
                                                db = "ncbi")
