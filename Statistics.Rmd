...
# Different distriutions are being driven by psyllid spp, with trioza having the best fit
...
## psyllid tree
p1 <- ggtree(newtree, ladderize=FALSE) + geom_tiplab()
...
#OTU tree
p2 <- ggtree(new_stree, ladderize=FALSE) + geom_tiplab(size = 1, align = TRUE, linesize=0.05, linetype="dotted")
...
dd <- assoc %>%
  ...
  left_join(res %>% 
              ...
              mutate(label.y = str_replace_all(label.y, "-", "_"))%>%
              mutate(label.y = str_replace_all(label.y, " ", "_"))) %>%
  ...
  mutate(highlight = case_when(
    str_detect(label.y, "Candidatus_Carsonella") ~ TRUE,
    TRUE ~ FALSE
  ))

d1_new <- dd %>% 
  dplyr::select(assoc, label.x, y.x, highlight, values) %>%
  dplyr::rename(label = label.x, y = y.x)%>%
  mutate(tree = "host") 
  
d2_new <- dd %>% 
  dplyr::select(assoc, label.y, y.y, highlight, values) %>%
