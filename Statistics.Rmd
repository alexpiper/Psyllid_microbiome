...
gg.tangle <- bind_rows(d1_new, d2_new) %>%
  ...
  mutate(y = y / max(y)) %>%
  ungroup %>%
  ggplot(aes(x=tree, y=y, group=assoc, colour=as.factor(signif))) +
    geom_line(alpha=0.1) +  
    scale_color_manual(values=c("steelblue", "darkorange1")) +
...
gg.hlinks <- d1_new %>%
  ...
  labs(y = "# Signficant links", x=NULL, title ="Psyllid taxa") + 
  coord_flip()
...
gg.slinks <- d2_new %>%
  ...
  theme_classic()+ 
  #theme(axis.text.x = element_text(angle=45, hjust=1)) +
  labs(y = "# Signficant links", x=NULL, title = "Top 50% Bacterial genera") + 
  coord_flip()

gg.hlinks + gg.slinks

pdf(file="figs/microbiome_fit_summary.pdf",  width = 8, height = 11, paper="a4")
...
gg.tangle <- bind_rows(d1_new, d2_new) %>%
  ...
  ungroup %>%
  #mutate(values = as.numeric((1-values)/100)) %>%
  ggplot(aes(x=factor(tree, levels=c("microbe", "host")), y=y, group=assoc,colour=as.factor(signif))) +
    geom_line(alpha=1) +  
    scale_color_manual(values=c("steelblue", "darkorange1")) +
    scale_x_discrete(expand = c(0,0)) + 
    theme_void() +
