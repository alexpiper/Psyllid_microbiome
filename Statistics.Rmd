...
sum_reads <- speedyseq::psmelt(ps2) %>%
...
lipa_out <- cbind(lipa$lipa %>%
                    as.data.frame %>%
                    rename_all(funs(str_replace(., ".", "stat_"))),
                  lipa$p.value %>%
                    as.data.frame %>%
                    rename_all(funs(str_replace(., ".", "pval_")))
                               )%>%
  rownames_to_column("measure")
...
#Write out
write_csv(BTDD_pm,"output/phylosymbiosis/BTDD.csv")
...
Colour nodes by the average of the tip values in GGtree
Add grey colour to middle of label
Plot higher ranks on tree (See how phyloseq plot tree does it)

Could use recursion function that i used for get_lineage_ott to calculate nodes
