...
```{r agglomerate}
#ps.coda <- tip_glom(ps2.prev, h=0.3) # Work out what h=0.3 approximates

genglom <- tax_glom(ps2, "Genus", NArm = TRUE)
tipglom <- tip_glom(ps2, h=0.3) # Work out what h=0.3 approximates
tipglom2 <- tip_glom(ps2, h=0.2) # Work out what h=0.3 approximates
...
p3tree = plot_tree(genglom, method = "treeonly",
                   ladderize = "left", title = "By Genus") +
  theme(plot.title = element_text(size = multiPlotTitleTextSize))

p4tree = plot_tree(tipglom, method = "treeonly",
                   ladderize = "left", title = "H=0.3") +
  theme(plot.title = element_text(size = multiPlotTitleTextSize))


p5tree = plot_tree(tipglom2, method = "treeonly",
                   ladderize = "left", title = "H=0.2") +
  theme(plot.title = element_text(size = multiPlotTitleTextSize))


library(patchwork)

p2tree + p3tree +  p5tree
...
## Test for microbiome distinguisability
...
ps2_bar <- #subset_taxa(ps2, !Family == "Enterobacteriaceae") %>%
            ps2 %>%
  speedyseq::tax_glom(taxrank = "Phylum") %>%           # agglomerate at Order level
...
col <- colorRampPalette(brewer.pal(11, "Spectral"))(28)

gg.bar <- ggplot(ps2_bar, aes(x = labels, y = Abundance, fill = Phylum)) + 
