...
# print phylogenetic signal
signal_rare
write_csv(signal_rare, "output/phylosymbiosis/alpha/phylosignal_rarefied.csv")
...
write_csv(mantel_results %>% dplyr::select(-pval1,-pval2), "output/phylosymbiosis/mantel_fulldata.csv")
...
s_dist <- cophenetic(s_tree) /1e+6 #convert to Mya so integers are small enough for PACO
...
paco_run <- readRDS("output/cophylogeny/psyllid_microbiome/paco_run_microbiome.rds")

#print overall significance
print(paco_run$gof)

links <- data.frame(OTU=names(paco_run$jackknife$mean), values=unname(paco_run$jackknife$mean), upper=unname(paco_run$jackknife$upper)) %>%
 separate(OTU, into=c("Sample_Name", "OTU"), sep="-", extra="merge") %>%
  mutate(signif = case_when(
    upper < mean(.$values) ~ TRUE,
    upper > mean(.$values) ~ FALSE
  ))
...
#%>%
#  dplyr::mutate(values = scales::rescale(values, to=c(0,1))) 
...
p1 <- p1 %<+% weights_p1 + geom_tippoint(aes(colour=links)) +  scale_color_gradient(low="steelblue", high="darkorange1")
...
p2 <- ggtree(newtree, ladderize=TRUE, aes(colour=links)) + 
  geom_text2(aes(subset=!isTip, label=group), hjust=0, check_overlap=TRUE)
weights_p2 <- p2$data %>%
  left_join(links %>% 
    group_by(OTU) %>%
    summarise(values = mean(values)) %>%
...
weights_p2 <- weights_p2 %>%
  left_join(data.frame(node=weights_p2$node, links = weights_p2$node %>% purrr::map_dbl(average_descendants, tree=tree1, df=weights_p2)))

p2 <- p2 %<+% weights_p2  + geom_tippoint(aes(colour=links)) +  scale_color_gradient(low="steelblue", high="darkorange1")
...
dd <- obj$assoc %>%
  ...
  left_join(links %>% 
              dplyr::select(Sample_Name, OTU, values, signif) %>% 
...
d1_new <- dd %>% 
  dplyr::select(assoc, label.x, y.x, values, signif) %>%
  dplyr::rename(label = label.x, y = y.x)%>%
  mutate(tree = "host") 
  
d2_new <- dd %>% 
  dplyr::select(assoc, label.y, y.y, values, signif) %>%
...
gg.tangle <- bind_rows(d1_new, d2_new) %>%
  ...
  ggplot(aes(x=tree, y=y, group=assoc, colour=signif)) +
    geom_line(alpha=0.1) +  
...
gg.hlinks <- d1_new %>%
  ...
  labs(y = "Contribution", x=NULL, title ="Psyllid taxa")
...
gg.slinks <- d2_new %>%
  ...
  theme_classic()+ 
  theme(axis.text.x = element_text(angle=45, hjust=1)) +
  labs(y = "Contribution", x=NULL, title = "Top 25% Bacterial genera")

gg.hlinks / gg.slinks

pdf(file="figs/microbiome_fit_summary.pdf",  width = 11, height = 8, paper="a4r")
  plot(gg.hlinks / gg.slinks)
try(dev.off(), silent=TRUE)

# Add labels to the tanglegram to make sure they are lining up
```
...
paco_run <- paco_links(paco_run)
links <- data.frame(OTU=names(paco_run$jackknife$mean), values=unname(paco_run$jackknife$mean)) %>%
 separate(OTU, into=c("Sample_Name", "OTU"), sep="-", extra="merge") %>%
  dplyr::mutate(values = scales::rescale(values, to=c(0,1)))
...
p1 <- ggtree(tree1, ladderize=FALSE, aes(colour=links)) + 
  geom_tiplab(align=TRUE , offset=0.3, hjust=1, linetype = "dashed", linesize=0)
weights_p1 <- p1$data %>%
  left_join(links %>% #res
  group_by(Sample_Name) %>%
  summarise(values = mean(values)) %>%
...
gg.tangle <- bind_rows(d1_new, d2_new) %>%
  ...
  ungroup %>%
  #mutate(values = as.numeric((1-values)/100)) %>%
  ggplot(aes(x=factor(tree, levels=c("microbe", "host")), y=y, group=assoc, colour=values)) +
    geom_line(alpha=1) +  
    scale_color_gradient(low="steelblue", high="darkorange1") +
    scale_x_discrete(expand = c(0,0)) + 
    theme_void() +
...
# run paco
paco_run <- PACo(D, nperm=99, seed=949, method='quasiswap', symmetric=TRUE) #Symetric - is one meant to track the evolution of another?
...
tree2 <- obj[["trees"]][[2]]
p2 <- ggtree(tree2, ladderize=FALSE, aes(colour=links)) +
  geom_tiplab(align=TRUE , offset=0.02, hjust=1, linetype = "dashed", linesize=0)
weights_p2 <- p2$data %>%
  left_join(links %>% 
    group_by(psyllid_spp) %>%
    summarise(values = mean(values)) %>%
...
gg.triozid_tangle <- p2 + gg.tangle + (p1 + scale_x_reverse()) + plot_layout(widths = c(2, 1, 2))
gg.triozid_tangle

pdf(file="figs/triozid_tanglegram.pdf",  width = 8, height = 11, paper="a4")
...
Could just calculate the average dissimilarities across all, then look at the factors we are interested in, and do a statistical test between - Could actually do this way down in the beta diversity section

Then could move everything up to beta diversity to the bioinformatics section, as its all technically bioinformatics not statistics
