...
betadisper_results <- distlist %>%
  ...
  bind_rows(
    broom::tidy(anova(vegan::betadisper(y, metadata$psyllid_spp))) %>%
      dplyr::slice(1) %>% 
      mutate(term="psyllid_spp"),
    broom::tidy(anova(vegan::betadisper(y, metadata$hostplant_spp))) %>%
      dplyr::slice(1) %>% 
...
# Parafit
PF_run <- parafit(h_dist, s_dist, coocur, nperm=99, test.links=FALSE, silent=TRUE)
