...
# psyllid tree
p1 <- ggtree(pruned.tree2, ladderize=FALSE) + geom_tiplab()
...
# OTU tree
p2 <- ggtree(phy_tree(ps3), ladderize=FALSE) + geom_tiplab(size = 1, align = TRUE, linesize=0.05, linetype="dotted")
...
# Get cophenetic distance
plant_samples <- intersect(plant.tree$tip.label, samdf$hostplant_spp)
h_dist <- cophenetic(plant.tree)[plant_samples, plant_samples]

# psyllid dist
psyllid_samples <- intersect(pruned.tree$tip.label, samdf$psyllid_spp)
s_dist <- cophenetic(pruned.tree)[psyllid_samples, psyllid_samples ]

coocur <- coocur[psyllid_samples, plant_samples]
# prepare paco data
...
# Get rotations using cophylo then extract tree?
obj <- cophylo(tr1=plant.tree, tr2=pruned.tree, assoc=assoc, rotate=TRUE)

# plant tree
p1 <- ggtree(plant.tree, ladderize=FALSE) + geom_tiplab()
weights_p1 <- p1$data %>%
  left_join(res %>%
...
# psyllid_tree
p2 <- ggtree(pruned.tree, ladderize=FALSE) + geom_tiplab()
weights_p2 <- p2$data %>%
  left_join(res %>% 
    group_by(psyllid_spp) %>%
    summarise(values = mean(values)) %>%
...
dd <- assoc %>%
  as_data_frame() %>%
  magrittr::set_colnames(c("label.y", "label.x")) %>%
  left_join(res %>% 
