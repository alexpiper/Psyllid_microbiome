...
Mantel test
Partial mantel test
Heirarchial clustering, robinson foulds, matching cluster
...
distlist$WUnifrac <- as.matrix(phyloseq::UniFrac(ps2, weighted=TRUE, parallel = parallel))
...
pmantel_results <- distlist %>%
  map(function(x){
    run_pm(x, dists=dists, samples=samples)
...
mantel_results <- distlist %>%
  map(function(x) {
    bind_rows(
    mantel(x[samples, samples], phylo.dist[samples, samples])[c("statistic","signif", "permutations")],
    mantel(x[samples, samples], coi.dist[samples, samples])[c("statistic","signif", "permutations")],
    mantel(x[samples, samples], plant.dist[samples, samples])[c("statistic","signif", "permutations")],
    mantel(x[samples, samples], spat.dist[samples, samples])[c("statistic","signif", "permutations")]
    )
})  %>%
  bind_rows(.id="dist") %>%
      group_by(dist) %>% 
      mutate(dist2 = c("phylo.dist","coi.dist","plant.dist", "spat.dist"))

# Partial Mantel tests
mantel_dists <-  c("phylo.dist","coi.dist","plant.dist", "spat.dist")
pm_dists <- expand_grid(dist1 = c("phylo.dist","coi.dist", "plant.dist", "spat.dist"), dist2=c("phylo.dist","plant.dist", "spat.dist")) %>%
  filter(!dist1==dist2)

# get a type argument here? if type == mantel, do mantel with dist1, if type = partial.mantel, do pm with dist1 + dist2
...
ddpcoa coloured by relevant covariate. Report results of mantel tests Robinson foulds etc. Then follow this up with the BDTT Analysis with just partial mantels 
