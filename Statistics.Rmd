...
```{r prepare distances}
#Filter to those in ps2 - shouldnt need to do this as we are filterign later!
...
distlist$Bray <- as.matrix(vegdist(otutab, method="bray"))
distlist$Aitchison <- as.matrix(vegdist(CoDaSeq::codaSeq.clr(otutab_n0), method="euclidean"))
#distlist$Philr <- as.matrix(vegdist(philr::philr(otutab_n0, phy_tree(ps2_noent),
#                                                part.weights='enorm.x.gm.counts',
#                                                ilr.weights='blw.sqrt'), method="euclidean"))
distlist$Unifrac <- as.matrix(phyloseq::UniFrac(ps2_noent, weighted=FALSE, parallel = TRUE))
distlist$WUnifrac <- as.matrix(phyloseq::UniFrac(ps2_noent, weighted=TRUE, parallel = parallel))
