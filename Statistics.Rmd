...
Now that we have removed the badly assigned taxonomy, we will make a new phylogenetic tree, constrianed by the taxonomy

Methods:
1- Align with silva online
2- Retrieve all nearest neighbours in alignment
3- Make tree in fasttree using constraints of taxonomy
4- Reroot tree
5- Date with PATHd8 
using dating anchors of: 
https://www.nature.com/articles/s41598-019-54869-6#Sec30 Supplementary table 5: https://static-content.springer.com/esm/art%3A10.1038%2Fs41559-018-0625-0/MediaObjects/41559_2018_625_MOESM1_ESM.pdf

also :https://static-content.springer.com/esm/art%3A10.1038%2Fncomms14319/MediaObjects/41467_2017_BFncomms14319_MOESM1451_ESM.pdf


would be better to use the SINA algorithm for this, as its recommended in the BDTT papers
https://github.com/epruesse/SINA

see paper: https://www.nature.com/articles/ncomms14319#Sec12
...
# Make constraints table at phylum level
taxonomy <- as.data.frame(tax_table(ps2)) %>%
  dplyr::select(-rootrank)
...
Phylum <- subset(Phylum, !Phylum %in% c("d__Bacteria")) #remove this factor
...
constraints=taxonomy[,c("Bacteria", as.character(Phylum))] #keep only the constrains 
head(constraints)

# Output fasta
seqs <- list()
for (i in 1:nrow(constraints)){
  seqs[[i]] <- constraints[i,]
  }
write.fasta(seqs, names=rownames(constraints), file.out="output/phylogenetic_constraints.fasta", open = "w", nbchar = 60, as.string = FALSE)


test <- charlist[intersect(tree$tip.label, rownames(seqtab_final))]

#Subset charlist to the original sequences, retain names
tree <- read.tree("arb-silva.de_2020-04-16_id809405/arb-silva.de_2020-04-16_id809405.tree")
library(ape)

samples <- intersect(tree$tip.label, rownames(seqtab_final))
tree2 <- drop.tip(tree, tree$tip.label[!tree$tip.label %in% samples])
...
compare with, without constraints, and to silva tree
...
sespd <- picante::ses.pd(comm, phy, null.model = "taxa.labels", include.root = F, runs = 10)
...
#tree1  <- as.phylo(hclust(as.dist(phylo.dist[subsample,subsample]), method = "ward.D2"))
#tree2 <- as.phylo(hclust(as.dist(distlist$Aitchison[subsample,subsample]), method = "ward.D2"))
...
gg.mantels <- bind_rows(mantel_results, pmantel_results) %>%
  ...
  filter(dist == "Aitchison") %>%
              ggplot(aes(x=dist1, y=mantelr, group=type, fill=type)) + 
...
# Plot circular phylogeny
gg.tree <- ggtree(phy_tree(ps2),  layout='circular') #branch.length='none',

gg.tree

# could colour by nodes most associated with the cofactor?
```
