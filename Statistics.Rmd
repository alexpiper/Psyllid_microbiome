...
# Count number of carsonella OTUs per psyllid species
cars <- speedyseq::psmelt(ps2) %>%
  filter(Abundance > 0) %>%
  group_by(psyllid_spp) %>%
  summarise(n = count(genus=="Candidatus Carsonella"))
...
report::report(aov(alpha ~psyllid_genus, data=div_table))
report::report(aov(Shannon ~psyllid_genus, data=div_table))
report::report(aov(pd ~psyllid_genus, data=div_table))

# Difference between genus/geography factors ANOVA
report::report(aov(alpha ~genus_geo, data=div_table))
report::report(aov(Shannon ~genus_geo, data=div_table))
report::report(aov(pd ~genus_geo, data=div_table))

#Test heirarchail
broom::tidy(aov(alpha ~psyllid_genus+psyllid_spp+psyllid_genus:psyllid_spp, data=div_table))


## Major genera only
# Difference between all major genera for alpha diversity ANOVA
div_table2 <- div_table %>%
  dplyr::filter(psyllid_genus %in% c("Trioza", "Ctenarytaina", "Psylla"))

mg_div <- bind_rows(broom::tidy(TukeyHSD(aov(alpha ~psyllid_genus, data=div_table2))) %>% mutate(type="Richness"),
          broom::tidy(TukeyHSD(aov(Shannon ~psyllid_genus, data=div_table2))) %>% mutate(type="Shannon"),
          broom::tidy(TukeyHSD(aov(pd ~psyllid_genus, data=div_table2))) %>% mutate(type="Phylogenetic"),
          broom::tidy(TukeyHSD(aov(alpha ~genus_geo, data=div_table2))) %>% mutate(type="Richness"),
          broom::tidy(TukeyHSD(aov(alpha ~genus_geo, data=div_table2))) %>% mutate(type="Shannon"),
          broom::tidy(TukeyHSD(aov(pd ~genus_geo, data=div_table2))) %>% mutate(type="Phylogenetic")
          )
write_csv(mg_div, "output/alpha/major_genera_alpha.csv")

# Difference between genus ANOVA
report::report(aov(alpha ~psyllid_genus, data=div_table2))
report::report(aov(Shannon ~psyllid_genus, data=div_table2))
report::report(aov(pd ~psyllid_genus, data=div_table2))

# Difference between genus/geography factors ANOVA
report::report(aov(alpha ~genus_geo, data=div_table2))
report::report(aov(Shannon ~genus_geo, data=div_table2))
report::report(aov(alpha ~genus_geo, data=div_table2))
...
mg_div_rare <- bind_rows(
  broom::tidy(TukeyHSD(aov(alpha ~psyllid_genus, data=div_table_rare2))) %>% mutate(type="Richness"),
  broom::tidy(TukeyHSD(aov(Shannon ~psyllid_genus, data=div_table_rare2))) %>% mutate(type="Shannon"),
  broom::tidy(TukeyHSD(aov(pd ~psyllid_genus, data=div_table_rare2))) %>% mutate(type="Phylogenetic"),
  broom::tidy(TukeyHSD(aov(alpha ~genus_geo, data=div_table_rare2))) %>% mutate(type="Richness"),
  broom::tidy(TukeyHSD(aov(alpha ~genus_geo, data=div_table_rare2))) %>% mutate(type="Shannon"),
  broom::tidy(TukeyHSD(aov(pd ~genus_geo, data=div_table_rare2))) %>% mutate(type="Phylogenetic")
          )
write_csv(mg_div_rare, "output/alpha/major_genera_alpha_rarefied.csv")

# Difference between genus factors ANOVA
report::report(aov(alpha ~psyllid_genus, data=div_table_rare2))
report::report(aov(Shannon ~psyllid_genus, data=div_table_rare2))
report::report(aov(pd ~psyllid_genus, data=div_table_rare2))

# Difference between genus/geography factors ANOVA
report::report(aov(alpha ~genus_geo, data=div_table_rare2))
report::report(aov(Shannon ~genus_geo, data=div_table_rare2))
report::report(aov(alpha ~genus_geo, data=div_table_rare2))
...
# Try heirarchial effects here, starting with the techical than moving down the phylogeny
#seqrun, psyllid
#this marginalises the effects of sample metadat variables first, before partitioning hte remaining variance into that accounted for by host phylogeny
...
adonis_results <- distlist %>%
  purrr::map(function(x) {
    bind_rows(
    broom::tidy(adonis(x~psyllid_genus+psyllid_spp, method="euclidean", data=metadata)$aov.tab) %>% dplyr::slice(1),
...
#Test heirarchial
#AIC permanova https://github.com/kdyson/R_Scripts/blob/master/AICc_PERMANOVA.R

#ACcountign for seqrun then phylogyn
m1 <- adonis(distlist$Aitchison~seqrun+psyllid_family+psyllid_genus+psyllid_spp, method="euclidean", data=metadata, by="terms")

AICc.PERMANOVA(m1)

#Accountign for seqrun, hosptlant then species
m2 <- adonis(distlist$Aitchison~seqrun+hostplant_spp+psyllid_spp, method="euclidean", data=metadata, by=NULL)
AICc.PERMANOVA(m2)
...
write_csv(mantel_results %>%
            dplyr::select(-pval1,-pval2),  "output/phylosymbiosis/mantel_fulldata.csv")#only keep two sided P values
...
slices=seq(0, 2e+09 , 1e07) #1e07

treelist <- purrr::map(slices, ~{phy_tree(speedyseq::tree_glom(ps2, resolution = .x))}) %>%
  purrr::set_names(paste0("Timeslice = ", slices)) 
...
Betas <- BDTT(ps2, slices, metrics=c("Aitchison"), zeroes="Impute", multithread = 2, quiet=FALSE, progress=FALSE) # "Philr"
...
BTDD_pm <- Betas %>%
  ...
  left_join(purrr::map(slices, ~{
  data.frame(
    otus = ntaxa(speedyseq::tree_glom(ps2, resolution = .x)),
    slice = .x)          
             }) %>%
  bind_rows()) %>%
...
gg.BTDD <- BTDD_pm %>%
  ...
  geom_line(aes(y = otus/10000), colour="grey20")+  #Add lineage curve
  scale_y_continuous(sec.axis = sec_axis(~.*10000, name = "Number of Lineages")) + 
  scale_color_manual(values=c("#1f78b4", "#b2df8a","#a6cee3"))+
  scale_fill_manual(values=c("#1f78b4", "#b2df8a","#a6cee3"))+
  base_theme +
  theme(legend.position = "bottom")  + 
  labs(x = "Mya", y= "Mantel R")
...
pdf(file="figs/mantels.pdf",  width = 10, height = 8, paper="a4r")
  plot(gg.allmantels)
try(dev.off(), silent=TRUE)
```
...
# Read in filtered 
paco_run <- readRDS("output/cophylogeny/paco_psyllid_microbe_filtered_asym.rds")
# Read in unfiltered 
...
# psyllid_tree
tree1 <- obj[["trees"]][[1]]
tree1 <- groupOTU(tree1, .node=73) # Make the atmeto dotted to indicate outgroup was rescaled

p1 <- ggtree(tree1, ladderize=FALSE, aes(colour=links,linetype=group)) #+ geom_tiplab()
weights_p1 <- p1$data %>%
  left_join(links %>%
  group_by(Sample_Name) %>%
...
p1$data[p1$data$node %in% c(73), "x"] <- max(p1$data$x)
p1$data[p1$data$node %in% c(74), "x"] <- 0.2 #root
...
whole_tanglegram <- p1 + gg.tangle + (p2  + 
  geom_text2(aes(subset=!isTip, label=group %>% na_if(0)), hjust=0, check_overlap=TRUE) + scale_x_reverse())
whole_tanglegram
...
### Heatmap of coocurrance
```{r psyllid microbiome heatmap}
gg.heatmap <- dd %>%
...
```{r S-symbiont cophylo}
coocur <- ps2 %>%
  transform_sample_counts(function (x) x/sum(x)) %>%
  speedyseq::psmelt() %>%
  filter(genus %in% c("Arsenophonus", "Sodalis")) %>%
  dplyr::select(OTU, psyllid_spp, SampleID, Abundance) %>%
  mutate(OTU = OTU %>%
           str_replace_all(" |-", "_")) %>%
  dplyr::group_by(OTU, psyllid_spp) %>%
...
gg.secondary_tangle <- p1 + gg.tangle + (p2 + scale_x_reverse()) #+ plot_layout(widths = c(2, 1, 2))
gg.secondary_tangle

pdf(file="figs/secondary_tanglegram.pdf",  width = 8, height = 11, paper="a4")
  plot(gg.secondary_tangle)
try(dev.off(), silent=TRUE)
```
...
gg.treemap
pdf(file="figs/trioza_microbiome_heatmap.pdf",  width = 8, height = 11, paper="a4")
  plot(gg.treemap)
try(dev.off(), silent=TRUE)
```
## Trioza ~ Carsonella
...
## ctenarytaina ~ Microbe

```{r ctenarytaina microbe}
#Prepare co-occurance matrix
coocur <- ps3 %>%
...
# Get interaction-specific cophylogenetic contributions using jacknifing
##paco_run <- paco_links(paco_run)
#write_rds(paco_run, "output/cophylogeny/paco_ctenarytaina_microbe_filtered.rds")

# Read in filtered 
paco_run <- readRDS("output/cophylogeny/paco_ctenarytaina_microbe_filtered.rds")
# Read in unfiltered 
#paco_run <- readRDS("output/cophylogeny/paco_ctenarytaina_microbe.rds")
...
dir.create("output/cophylogeny/ctenarytaina_microbiome")
write_csv(links, "output/cophylogeny/ctenarytaina_microbiome/psyllid_microbiome_links.csv")
write_csv(links %>% 
    group_by(OTU) %>%
    summarise(values = mean(values)), "output/cophylogeny/ctenarytaina_microbiome/microbe_weights.csv")
write_csv(links %>% 
    group_by(Sample_Name) %>%
    summarise(values = mean(values)), "output/cophylogeny/ctenarytaina_microbiome/psyllid_weights.csv")
...
# Parafit run
#PF_run <- parafit(h_dist, s_dist, t(coocur), nperm=99, test.links=TRUE, silent=TRUE)
#write_rds(PF_run, "output/cophylogeny/parafit_ctenarytaina_microbe_filtered.rds")

# Read in filtered
PF_run <- readRDS("output/cophylogeny/parafit_ctenarytaina_microbe_filtered.rds")
# Read in unfiltered
#PF_run <- readRDS("output/cophylogeny/parafit_ctenarytaina_microbe_unfiltered.rds")
...
ctenarytaina_tanglegram <- p1 + gg.tangle + (p2 + scale_x_reverse() + 
  geom_text2(aes(subset=!isTip, label=group %>% na_if(0)), hjust=0, check_overlap=TRUE))
ctenarytaina_tanglegram

pdf(file="figs/ctenarytaina_microbe_tanglegram.pdf",  width = 8, height = 11, paper="a4")
  plot(ctenarytaina_tanglegram)
try(dev.off(), silent=TRUE)
...
gg.ctenarytaina_hlinks <- gg.para_hlinks + gg.paco_hlinks 

gg.ctenarytaina_slinks <- gg.para_slinks + gg.paco_slinks

pdf(file="figs/ctenarytaina_microbiome_host_fit_summary.pdf",  width = 8, height = 11, paper="a4")
  plot(gg.ctenarytaina_hlinks)
try(dev.off(), silent=TRUE)
  
pdf(file="figs/ctenarytaina_microbiome_symbiont_fit_summary.pdf",  width = 8, height = 11, paper="a4")
  plot(gg.ctenarytaina_slinks)
try(dev.off(), silent=TRUE)
```


### Heatmap of coocurrance
```{r ctenarytaina microbiome heatmap}
gg.heatmap <- dd %>%
  dplyr::rename(Sample_Name = label.x,
                OTU = label.y,
                pos_x = y.x,
...
gg.treemap
pdf(file="figs/ctenarytaina_microbiome_heatmap.pdf",  width = 8, height = 11, paper="a4")
  plot(gg.treemap)
try(dev.off(), silent=TRUE)
```
...
# Get interaction-specific cophylogenetic contributions using jacknifing
##paco_run <- paco_links(paco_run)
#write_rds(paco_run, "output/cophylogeny/paco_psylla_microbe_filtered.rds")

# Read in filtered 
paco_run <- readRDS("output/cophylogeny/paco_psylla_microbe_filtered.rds")
# Read in unfiltered 
#paco_run <- readRDS("output/cophylogeny/paco_psylla_microbe.rds")
...
dir.create("output/cophylogeny/psylla_microbiome")
write_csv(links, "output/cophylogeny/psylla_microbiome/psyllid_microbiome_links.csv")
write_csv(links %>% 
    group_by(OTU) %>%
    summarise(values = mean(values)), "output/cophylogeny/psylla_microbiome/microbe_weights.csv")
write_csv(links %>% 
    group_by(Sample_Name) %>%
    summarise(values = mean(values)), "output/cophylogeny/psylla_microbiome/psyllid_weights.csv")
...
# Parafit run
#PF_run <- parafit(h_dist, s_dist, t(coocur), nperm=99, test.links=TRUE, silent=TRUE)
#write_rds(PF_run, "output/cophylogeny/parafit_psylla_microbe_filtered.rds")

# Read in filtered
PF_run <- readRDS("output/cophylogeny/parafit_psylla_microbe_filtered.rds")
# Read in unfiltered
#PF_run <- readRDS("output/cophylogeny/parafit_psylla_microbe_unfiltered.rds")
...
psylla_tanglegram <- p1 + gg.tangle + (p2 + scale_x_reverse() + 
  geom_text2(aes(subset=!isTip, label=group %>% na_if(0)), hjust=0, check_overlap=TRUE))
psylla_tanglegram

pdf(file="figs/psylla_microbe_tanglegram.pdf",  width = 8, height = 11, paper="a4")
  plot(psylla_tanglegram)
try(dev.off(), silent=TRUE)
...
gg.psylla_hlinks <- gg.para_hlinks + gg.paco_hlinks 

gg.psylla_slinks <- gg.para_slinks + gg.paco_slinks

pdf(file="figs/psylla_microbiome_host_fit_summary.pdf",  width = 8, height = 11, paper="a4")
  plot(gg.psylla_hlinks)
try(dev.off(), silent=TRUE)
  
pdf(file="figs/psylla_microbiome_symbiont_fit_summary.pdf",  width = 8, height = 11, paper="a4")
  plot(gg.psylla_slinks)
try(dev.off(), silent=TRUE)
```


### Heatmap of coocurrance
```{r psylla microbiome heatmap}
gg.heatmap <- dd %>%
  dplyr::rename(Sample_Name = label.x,
                OTU = label.y,
                pos_x = y.x,
...
gg.treemap
pdf(file="figs/psylla_microbiome_heatmap.pdf",  width = 8, height = 11, paper="a4")
  plot(gg.treemap)
try(dev.off(), silent=TRUE)
```
