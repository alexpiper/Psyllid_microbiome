...
bt_host <- betta(chats = ba_estimates$estimate,
                      ses = ba_estimates$error,
                      X = model.matrix(~hostplant_spp, data = ba_estimates)) 

bt_hostdf <- as_tibble(bt_host$table, rownames="psyllid_spp")%>% 
                mutate_at(2:4, as.numeric) %>%
              magrittr::set_colnames(c("Estimates", "SE", "P"))
...
#Would you also add seqrun as a random effect? collection date?  
...
## Arrange this by phylogeny? then have the mantel and ID results?

## How to test if you can predict richness by identity?

# Richness by phylogeney? Mantel
...
```{r chao bunge}
cb <- breakaway(ps2 %>% subset_samples(SampleID=="88_S75")) 
...
As this study was a survey across many species with not many replicates, it is unsuprising that significant differences were not found. Therefore there is only limited assertions we can make make about underlying richness.
...
## Can microbiome predict Identity
...
dataMatrix <- data.frame(psyllid_spp = sample_data(ps.coda)$psyllid_spp, f.clr) 


# take 200 samples at random to be the training set, and the remaining 54 the test set
training <- sample(unique(sample_data(ps.coda)$SampleID), size = 200) 

inTrain <- which(sample_data(ps.coda)$SampleID %in% training)

training <- dataMatrix[inTrain,] %>%
  mutate(psyllid_spp = as.factor(psyllid_spp))
testing <- dataMatrix[-inTrain,] %>%
  mutate(psyllid_spp = as.factor(psyllid_spp))

plsFit <- train(psyllid_spp ~ ., data = training,
...
table(plsClasses, testing$psyllid_spp)

#Random forest
#training$psyllid_spp <- as.factor(training$psyllid_spp)

rfFit <- train(psyllid_spp ~ ., data = training, method = "rf",
...
gg.carson <- plot_tree(ps3.carson, ladderize="left",  color="psyllid_spp") +   theme(legend.position = "none") + #
