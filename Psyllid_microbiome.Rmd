...
assign_ranks <- fast_melt(ps) %>% 
  dplyr::select(c("taxaID","Root","Phylum","Class","Order","Family","Genus","Species")) %>%
  distinct()%>%
...
run2_pass <- run2_ps %>%
    ...
    transform_sample_counts(function (x) (x > 0.0035) * 1)
...
gg.bar <- ggplot(ps3_bar, aes(x = labels, y = Abundance, fill = plot_level)) + 
