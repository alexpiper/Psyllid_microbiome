...
tax <- t(sapply(ids, function(x) {
  ...
  magrittr::set_rownames(getSequences(test2)) %>%
...
saveRDS(tax, "output/rds/tax_IdTaxa_cut.rds") 
...
```{r LULU}
seqtab_final <- readRDS("output/rds/seqtab_nocut.rds")
tax_exact <- readRDS("output/rds/tax_IdTaxaExact_nocut.rds") 
...
# get mapping
curated_result <- readRDS("output/lulu_curated.rds")
...
#seqtab <- t(readRDS("output/rds/seqtab_final.rds"))
tax <- readRDS("output/rds/tax_IdTaxaExact_cut.rds")
