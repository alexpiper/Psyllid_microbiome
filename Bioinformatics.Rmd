...
# Output plot of ids
pdf(paste0("output/idtaxa.pdf"), width = 11, height = 8 , paper="a4r")
  plot(ids)
...
```{r phylogenetic tree}
seqtab_final <- readRDS("output/rds/seqtab_curated.rds")

seqs <- rownames(seqtab_final)
names(seqs) <- seqs # This propagates to the tip labels of the tree
seqs <- DNAStringSet(seqs)
writeXStringSet(seqs, "output/curated_asv.fasta", width=1000)

## SINA TREE AND ALIGNMENT

alignment <- insect::readFASTA("arb-silva.de_2020-04-16_id809405/arb-silva.de_2020-04-16_id809405_merged.fasta")

test <- 
charlist <- insect::dna2char(aphid::unalign(as.matrix(alignment)))

#Subset charlist to the original sequences, retain names
# Prune trees to charlist subset
# Replace names on trees with sequence
# Export for phyloseq
